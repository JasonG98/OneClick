...
def configs(name, settings):
    ...
    for mode in ("Debug", "Release"):
        ...
        values.update(
            ...
            CODE_SIGN_INJECT_BASE_ENTITLEMENTS="YES" if debug else "NO",
        )
        if debug:
...
assets = add(
    ...
    exceptions=[
        add(
            ...
            membershipExceptions=["icon.svg", "icon.png"],
            target=hashlib.sha256(b"App").hexdigest()[:24].upper(),
        )
    ],
)
extgroup = add(
...
extproduct = add(
    ...
    path="OneClickFinder.appex",
    sourceTree="BUILT_PRODUCTS_DIR",
)
products = add(
    "Products",
    "PBXGroup",
    children=[appproduct, extproduct],
    name="Products",
    sourceTree="<group>",
)
signing_config = add(
    ...
    path="config/Signing.xcconfig",
    sourceTree="<group>",
)
root = add(
    "RootGroup",
    "PBXGroup",
    children=[appgroup, extgroup, shared, assets, signing_config, products],
    sourceTree="<group>",
)
...
ext = add(
    ...
    "PBXNativeTarget",
    name="OneClickFinder",
    productName="OneClickFinder",
    productReference=extproduct,
    productType="com.apple.product-type.app-extension",
    buildPhases=phases("Ext"),
    buildRules=[],
    dependencies=[],
    ...
    buildConfigurationList=configs(
        ...
        dict(
            ...
            PRODUCT_BUNDLE_IDENTIFIER="local.oneclick.app.finder",
            INFOPLIST_FILE="config/FinderExtension-Info.plist",
            CODE_SIGN_ENTITLEMENTS="config/FinderExtension.entitlements",
...
app = add(
    ...
    "PBXNativeTarget",
    name="OneClick",
    productName="OneClick",
    productReference=appproduct,
    productType="com.apple.product-type.application",
    buildPhases=phases("App") + [embed],
    buildRules=[],
    dependencies=[dep],
    fileSystemSynchronizedGroups=[appgroup, shared, assets],
    buildConfigurationList=configs(
        "App",
        dict(
            common,
            PRODUCT_NAME="$(TARGET_NAME)",
            PRODUCT_BUNDLE_IDENTIFIER="local.oneclick.app",
            INFOPLIST_FILE="config/OneClick-Info.plist",
            CODE_SIGN_ENTITLEMENTS="config/OneClick.entitlements",
...
def encode(value, indent=0):
    ...
    if isinstance(value, dict):
        fields = "".join(
            f"{pad}\t{json.dumps(k)} = {encode(v, indent + 1)};\n"
