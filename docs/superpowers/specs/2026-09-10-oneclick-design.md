...
> **部分结论已被取代**，以 [后台形态与右键菜单优化方案](../plans/2026-09-10-background-runtime-and-menu.md) 为准：
>
> - **Claude Code 已移除**（暂时）。`.claude` 目标类型、`claude-cli://` 深链接以及"Terminal 与 Claude Code"整节都不再生效；预置目标改为 Terminal。
> - **"首版不增加菜单栏常驻图标"已被推翻**。入口改为 Finder 工具栏按钮，由已常驻的扩展提供，零额外进程。原文结论见下文"配置窗口具体包含"一节。
> - **"只把受沙盒限制的操作转交主应用"只落实了一半**：实际上所有打开动作都转交了主应用。当时未查明原因——`selectedItemURLs()` 交出的 URL 不带沙盒读取权限，任何应用都打不开，不只是 Terminal。现已用只读例外让扩展自足，整套请求中转机制删除。
> - **"选中文件时使用系统打开方式"保持不变**，这是明确决策而非遗留问题。
>
> 下文保留为当时的设计记录，不再逐条修订。
