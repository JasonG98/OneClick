...
## 2026-09-22：无账号构建与共享目录

环境：本机 macOS 27.0（26A428）、Apple Silicon、Xcode 27.0（27A266a）。
以下是本次版本的证据；后面的 2026-09-10 起记录是历史情况，其中团队签名、App Group、
主程序转发打开动作等旧方案不能视为当前要求。

| 项目 | 本次结果 |
| --- | --- |
| 自动化 | `./script/check.sh --build --disable-sandbox` 通过：96 项 Swift 测试、当时 50 项 Python 脚本测试、图标逐字节校对、Shell 语法、arm64 主程序/扩展编译。后续增加 `--verify` 回归用例，脚本测试为 51 项，最终全套测试通过。 |
| 无团队构建 | `build_and_run.sh --build-only` 与 `release.sh 0.1.0` 实际完成；旧 Local.xcconfig 仍在，但不被构建读取。主程序和扩展 `Signature=adhoc`、`TeamIdentifier=not set`。 |
| 发布权限 | 实际 Release 扩展含 app-sandbox、原有 `/` 只读例外和 `/Library/Application Support/OneClick/` 的 home-relative read-write；没有 application-groups，也没有 get-task-allow。 |
| Finder 加载 | `build_and_run.sh --verify` 实际启动 app 与扩展；PluginKit 指向当前 Debug `.appex`，进程存在。扩展日志记录 `Observing 1 configured roots`、`Finder extension initialized` 和开始监控目录，说明读取到了主程序创建的新配置。 |
| 真实 home | 同生产源码的沙盒探针输出共享路径为账户实际 home 下的 `Library/Application Support/OneClick`；Foundation 默认 home 则在 `Library/Containers/local.oneclick.storage-probe/Data`，验证了不能用默认 home 的原因。 |
| 沙盒读写正例 | 完整 `.app` 探针以 ad hoc 和生产 FinderExtension.entitlements 签名，编入生产 SharedDirectory、SharedEnvironment、SettingsRepository；在共享目录的独立临时子目录完成配置原子写入/读取、错误原子写入/读取/消费；临时子目录自动清理。 |
| 沙盒边界及对照 | 相邻 Application Support 路径写入被拒绝。移除唯一的 home-relative read-write 例外、用独立 bundle id 运行对照探针后，在 OneClick 目录中创建临时子目录也被拒绝（退出 1）。这证明例外在本机实际生效，未扩大到整个 Application Support。 |
| DMG | `hdiutil verify` 通过；只读挂载后核对 OneClick.app、`Applications -> /Applications` 和 INSTALL.txt；镜像内 app 的 `codesign --verify --deep --strict` 通过，嵌套扩展为 arm64/ad hoc。检查后已卸载镜像。 |
| Gatekeeper | `spctl --assess --type execute --verbose=2` 对最终 Release app 返回 `rejected`（退出 3），与未公证预期一致；没有修改 Gatekeeper 或 TCC 设置。 |
| Cask / 工作流 | 最终 DMG 实测 SHA-256 用于生成 `dist/oneclick.rb`，Ruby 语法通过；两个 workflow YAML 解析通过。外部命令替身覆盖 draft 创建、重跑、公开版本拒绝修改及无账号构建所有运行模式。 |

**两个假设的状态：**

- **沙盒扩展的读写例外：本机已验证。** 有真实沙盒生产源码正反对照，也有真实 Finder 扩展读取配置的证据。macOS 26 的干净安装还需复核。
- **ad hoc 且无 App Group 后彻底不再出现“访问其他 App 数据”：未完整验证。** 本机有旧版使用历史，无法代表干净账户。Computer Use 初次请求未返回，用户要求重试后返回 ScreenCaptureKit `-3811`（音频/视频捕捉失败），因此没有取得可用的桌面观察证据，不能承诺任何情况下均无弹窗。正常启动代码已不再访问或扫描 App Group。

仍需人工验收：干净账户上安装/启用/反复重建/重启后的提示情况；真实 Finder 菜单点击、
关闭设置后的 Terminal/编辑器打开；浏览器下载带 quarantine 的 DMG 后按 README 解锁；
公开 tap 的安装、升级、卸载。GitHub Actions 和远程 draft 在本次没有触发，未推送 tag 或发布。

本机可复查的日志位于 `.build/logs/`：`final-check.log`、`final-tests.log`、
`final-runtime-verify.log`、`storage-probe.log`、`storage-probe-readonly.log`、
`release-build.log`、`gatekeeper.log`。日志与探针都是忽略的本地验证产物，不随应用分发。

本轮遇到的环境问题：外层沙盒禁止 Swift 宏子进程启动，Xcode 构建改在获准的外层环境运行后通过；
最初把沙盒探针作为裸可执行文件启动，退出于 AppSandbox 初始化，改为带 Info.plist 的完整 `.app`
后正例通过。两者均未通过放宽生产 entitlement 来绕过。

最后的重建还遇到过一次扩展未恢复：日志显示它先启动，随后 PluginKit 查询无记录；
手动重新登记同一份 ad hoc `.appex` 后恢复。结合旧构建注销顺序，推断是异步注销与新登记的时序问题。
脚本现于原来的 10 秒等待窗口内每 2 秒重新登记一次；“延迟登记才恢复”的隔离回归先失败后通过，
最终 `build_and_run.sh --verify` 返回 0，主程序和扩展均存活。超时仍返回非零，不能把失败藏成成功。
